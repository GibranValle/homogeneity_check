...
export const Editor: FC = () => {
	...
	return (
		...
		>
			<Box sx={{ width: '95%' }}>
				<Typography variant="h6">{`Window center: ${center}`}</Typography>
				<Slider
					sx={{
						p: 2,
						width: '90%',
			...
			<Box sx={{ width: '95%' }}>
				<Typography variant="h6">{`Window width: ${width}`}</Typography>
				<Slider
					sx={{
						p: 2,
						width: '90%',
			...
			<Box sx={{ display: 'flex', justifyContent: 'space-around', width: '100%' }}>
				<Button variant="contained" onClick={handleOriginal}>
					Original
				</Button>
			...
			<Box sx={{ display: 'flex', justifyContent: 'space-around', width: '100%', gap: 2, mt: 1 }}>
				{ZOOM_LIST.map((item, index) => (
					<Button fullWidth key={`bz-${index}`} title={`${item}`} color="secondary" variant="contained" onClick={() => updateZoom(item)}>
						<SearchIcon /> {item}%
					</Button>
				))}
			</Box>
			...
			<Box sx={{ display: 'flex', justifyContent: 'space-around', width: '100%', gap: 2, mt: 1 }}>
				<Button fullWidth variant="contained" onClick={handleCenter}>
					<ControlCameraIcon /> Centrar
				</Button>
			</Box>
		</Box>
	)
