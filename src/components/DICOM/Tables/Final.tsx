...
export const Final = ({ statistics }: Props) => {
	return (
		<TableContainer component={Paper} elevation={3} sx={{ p: 1, mb: 2 }}>
			<Table>
				<TableHead>
					<TableRow>
				...
				<TableBody>
					{statistics.map((item, index) => (
						<TableRow key={`t2-r-${index}`}>
							<TableCell>
								<Typography align="center" variant="h6" color={item.color}>
									{item.roi}
								</Typography>
							</TableCell>
							<TableCell>
							...
							<TableCell>
								<Typography align="center" variant="h6" color={Math.abs(item.stdDevRSR) < 20 ? 'green' : 'red'}>
									{Math.abs(item.stdDevRSR) < 20 ? SUCCESS : FAIL}
								</Typography>
							</TableCell>
						</TableRow>
					))}
